...
async function main() {
  ...
  logger.info(`Unshielded coins count: ${initialState.availableCoins.length}`);
  logger.info(`Unshielded coins: ${JSON.stringify(initialState.availableCoins)}`);
  logger.info(`Unshielded balance: ${initialState.balances[token.raw]}`);

  try {
    const dustAddress = await walletProvider.wallet.dust.getAddress();
    logger.info(`Dust address: ${dustAddress.toString()}`);
  } catch (err: any) {
    logger.error(`Error getting dust address: ${err?.message || err}`);
  }

  const currentState = await Rx.firstValueFrom(walletProvider.wallet.state());
  logger.info(`Current dust balance: ${currentState.dust.balance(new Date())}`);
  logger.info(`Current dust available coins: ${JSON.stringify(currentState.dust.availableCoins)}`);
